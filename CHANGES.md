...
- `disko_bayes --sequential` chains turns in the low-rank information form (`SequentialInfoState`): the posterior is carried as `s0^{-2} I + sum V_1 B V_1^T` and is never converted to (or rotated between) sky-space covariance matrices, removing the per-step O(n_s^3) natural-basis round trips and n_s x n_s allocations. It computes the exact conjugate-Gaussian posterior for the sequence of reduced operators (the previous per-turn block chaining dropped the prior's range-null cross-correlations whenever successive turns have different natural bases); sky-space mean/variance/pcf images are materialized only when requested, and via matrix-free `pcf_row` for the PCF
