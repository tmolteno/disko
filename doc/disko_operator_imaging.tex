...
the prior of the next snapshot (\texttt{disko\_bayes --hdf} loops over
visibilities, chaining \texttt{posterior $\to$ prior}). Chained posteriors are
dense, so the general (exact) path is used for them; correctness is asserted in
\texttt{test\_do\_inference\_sequential}. The package also draws posterior
samples (with results written as \texttt{*\_mu}, \texttt{*\_var},
...
analytic sequential updates. For problems too large for an explicit SVD, the
