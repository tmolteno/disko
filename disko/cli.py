...
def get_source_list(source_json, el_limit, jy_limit):
    ...
    if source_json is not None:
        src_list = elaz.from_json(source_json, el_limit_deg=el_limit, jy_limit=jy_limit)
    return src_list
...
def main():
    ...
    algo_group.add_argument(
        "--lsqr", action="store_true", help="Use lsqr in matrix-free"
    )
    algo_group.add_argument(
        "--lsmr", action="store_true", help="Use lsmr in matrix-free"
    )
    algo_group.add_argument(
        "--fista", action="store_true", help="Use FISTA in matrix-free"
    )
    algo_group.add_argument(
        "--lasso", action="store_true", help="Use L1 regularization."
    )
    algo_group.add_argument(
        "--tikhonov", action="store_true", help="Use L2 regularization."
    )

    parser.add_argument(
        "--matrix-free", action="store_true", help="Use matrix-free regularization."
    )
    parser.add_argument(
        "--niter",
        type=int,
        default=100,
        help="Number of iterations for iterative solutions.",
    )

    parser.add_argument("--dir", required=False, default=".", help="Output directory.")
    parser.add_argument(
        "--alpha", type=float, default=None, help="Regularization parameter."
    )
    parser.add_argument(
        "--l1-ratio",
        type=float,
    ...
    parser.add_argument(
        "--HDF", required=False, help="Generate a HDF format field of view."
    )
    parser.add_argument(
        "--VTK", action="store_true", help="Generate a VTK mesh format image."
    )
    parser.add_argument(
    ...
    if ARGS.lasso:
        ...
        sky = disko.image_tikhonov(
            disko.vis_arr, sphere, alpha=ARGS.alpha, scale=False, usedask=ARGS.dask
        )
